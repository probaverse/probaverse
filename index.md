...
The probaverse is designed for anyone who wants to work with probability
distributions in R. It is particularly useful for data scientists,
statisticians, and researchers who require the flexibility to develop
advanced statistical models. Special focus is put on risk and hazard
modelling, where distributions are heavily used in practice; therefore,
those in fields such as finance, insurance, environmental science, and
engineering may find the probaverse particularly appealing.
...
- Implementing multivariate distributions, especially through copulas
  (new `couple` package).
- Extending options for fitting non-parametric distributions (new
  `gristmill` package).
...
[contributing guide](https://probaverse.probaverse.com/CONTRIBUTING.md)
to get started.
...
#> ✔ famish       0.1.0.9000   Flexibly Tune Families of Probability Distributions
```
...
ROpenSci for their insightful feedback in the core `distionary` package
that powers the probaverse suite. Lastly, thanks to the developers of
`tidyverse` for providing a model for cohesive design and appropriate
structure of a meta-package.

## Code of Conduct

Please note that the probaverse project is released with a [Code of
Conduct](https://probaverse.probaverse.com/CODE_OF_CONDUCT.md). By
